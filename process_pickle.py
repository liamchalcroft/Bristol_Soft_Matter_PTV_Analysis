...
def process_pickle(file_path, general_directory, microns_per_pixel, timestep, frame_lookback):
...
#     df['biny'] = (df['y']/bin_width).apply(np.floor)*bin_width
    ...
    def bins(width, no_bins):
        markers = []
        bin_width = width / no_bins
        for i in range(no_bins):
            markers.append((i*bin_width, (i+1)*bin_width))
        return markers

    no_bins = 20
    mybins = bins(width, no_bins)

    def find_bin(x_pos, bins):
        for counter, x in enumerate(bins):
            if x_pos >= x[0] and x_pos < x[1]:
                return counter        

    def map_bin(data):
        return find_bin(data, mybins)
    ...
    f.write("\n")
    f.write("Average particle velocity - position difference method: {}".format(df.hyp_vel.mean()))
    f.write("\n")
    f.write("Average value of ep: {}".format(df.ep.mean()))
    f.write("\n")
    f.write("Standard Deviation in Velocity by Bin: {}".format(df.groupby('bin').hyp_vel.std()))
    f.write("\n")
    f.write("Average standard dev: {}".format(df.groupby('bin').hyp_vel.std().mean()))
    f.write("\n")
    f.write("Frame lookback: {}".format(frame_lookback))
    f.close()
